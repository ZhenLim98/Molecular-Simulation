function [ nSteps, t, E_t, T_t, R2_t, D_t, Fmax_t, Frms_t] = langevinMD_PBC4_2( nAtoms, gamma, targetTemp, timestep, equilTime, runTime, nocells1D, cutoff_radius, h, mdmode, startconf)
%LANGEVINMD Perform Langevin Molecular Dynamics on a strand of polymer
% Example command:  [n t E T R2 D Fmax Frms] = langevinMD_PBC4_2(2,0,300.0,1,1000.,10000.,4,10,[25 0 0; 0 25 0; 0 0 25],'constantE','new')
%   Input variables:
                    ...
                    for l=1:nAtoms
                        for i1=1:nAtoms
                            if l == i1 && p == 0 && q == 0 && r == 0 
                                ...
                                d_origin2 = (pos(i1,1) + component_x) * (pos(i1,1) + component_x) ...
                                    + (pos(i1,2) + component_y) * (pos(i1,2) + component_y) ...
                                    + (pos(i1,3) + component_z) * (pos(i1,2) + component_z);
                                
                                if d_origin2 < cutoff_radius * cutoff_radius
                                    r2 = (pos(l,1) - pos(i1,1) - component_x)^2 ...
                                + (pos(l,2) - pos(i1,2)- component_y)^2 ...
                                + (pos(l,3) - pos(i1,3)- component_z)^2 ;
                                x2 = sigma*sigma/r2;
                                x6 = x2*x2*x2; 
                                x12 = x6*x6;
                   ...
                   for r = -nocells1D:nocells1D
                        for j2=1:nAtoms
                            if  i2 == j2 && p == 0 && q == 0 && r == 0
                                ...
                                component_y = p * h(2,1) + q * h(2,2) + r * h(2,3);
                                component_z = p * h(3,1) + q * h(3,2) + r * h(3,3);
                                dPos(1) = pos(i2,1) - pos(j2,1) - component_x;
                                dPos(2) = pos(i2,2)- pos(j2,2) - component_y;
                                dPos(3) = pos(i2,3)- pos(j2,3) - component_z;
                                r2 = dPos(1)^2 + dPos(2)^2 + dPos(3)^2 ;
                                %if beyond a certain radius then stops
                                %counting interactions
                                d_origin2 = (pos(j2,1) + component_x)*(pos(j2,1) + component_x) ...
                                    + (pos(j2,2) + component_y) * (pos(j2,2) + component_y) ...
                                    + (pos(j2,3) + component_z) * (pos(j2,2) + component_z);
                                if d_origin2 < cutoff_radius * cutoff_radius
                                x2 = sigma*sigma/r2;
                                x6 = x2*x2*x2;
                                x12 = x6*x6;

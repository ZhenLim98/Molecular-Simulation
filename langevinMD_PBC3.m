...
%
  ...
  % only - with PBCs such that it also interacts with immediate neighbours 
...
%
    function potential2 = getNonBondingEnergy ()
        ...
        %   for all particles with each other
        % for a system of side length nocells1D - 1 
    ...
    function temperature = getTemperature ()
        ...
        %calculate for one unit cell only as temperature is intensive variable
        vcm = mean(vel);
...
%          end
        ...
        % Add the non-bonding contribution to the atomic forces between

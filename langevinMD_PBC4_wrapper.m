function [ nSteps, t, E_t, T_t, R2_t, D_t,Fmax_t, Frms_t] = langevinMD_PBC4_wrapper( nAtoms, gamma, targetTemp, P_target, timestep, equilTime, runTime, nocells1D, h,V,mdmode, startconf)
...
%   V               initial value of In P-R (at least) its =  h_dot h^-1 where h_dot is the
%                   matrix of unit cell vector velocities (in columns) and
%                   h is given above (Note: initially symmetric)
%   P_target        Target pressure (what units?)       
%                  
%
%   mdmode:
...
%             quarterstep = 0;
            ...
            halfstep = 0;
            quarterstep = 1;
            V_update(halfstep,quarterstep, A_v)
    ...
    function Molec_velocities_qq()
      ...
      factor2 = C \(eye(3) - expm(-timestep * C));
...
%     s_dot = h \ vel;
%     %s_dot_dot is factor 3
%     factor3 = 1/atomicmass * (h \ force) - (G \  G_dot) * s_dot;    
%     %propagating using Louvillian expression.
%     vel = vel + timestep *( h_dot * s_dot +  h * factor3); 
    end

    function V_update(halfstep,quarterstep, A_v)   
    %constructing A_r
    S = getStressTensor();
    [s,~] = size(S);
    A_r = det(h)*( (S - eye(s) * P_target))/W ;
...
%     end
    ...
    %constructing matrix of normalised eigenvectors of V
    
    [~,eig_matrix] = eig(V);
    [~,no_of_eigenvectors] = size(eig_matrix);
    for d1 = 1:1:no_of_eigenvectors

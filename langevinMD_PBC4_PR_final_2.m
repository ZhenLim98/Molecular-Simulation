function [ nSteps, t, E_t, T_t, R2_t, D_t, Fmax_t, Frms_t, P_t, K_t, Vol_t, P_current, H_t] = langevinMD_PBC4_PR_final_2( nAtoms, gamma, targetTemp, timestep, equilTime, runTime, nocells1D, cutoff_radius, h, V, P_target, mdmode, startconf)
...
%
    function moveAtoms ()
        ...
        elseif intalgo == 3
            ...
            S = getStressTensor();%
            K = getKinetic();
            v3q = v2q + factor2*(force/atomicmass - V * vel - 1/N_f * trace(V) * vel );%
            V3q = V2q + factor2*( 1/W * det(h) * (S - I * P_target) + 1/(W * N_f) * 2 * K * I);%
